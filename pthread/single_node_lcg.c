...
//#define PIN_BFS_PROCESS

#ifdef PIN_BFS_PROCESS
int *monitor_enter_ptr = NULL;


void set_enter_bfs_process()
{
    //double a = sin(90);
    monitor_enter_ptr = (int*)realloc(NULL, 1); 
    fprintf(stdout,"enter into bfs process: 0x%lx\n", monitor_enter_ptr);
    fflush(stdout);
}
void reset_leave_bfs_process()
{
    //double b = cos(90);
    monitor_enter_ptr = (int*)realloc(NULL, 0); 
    fprintf(stdout, "leave bfs process: 0x%lx\n", monitor_enter_ptr);
    fflush(stdout);
}
#endif
